...
with open(zig_filename, "w") as f:
    ...
    f.write("    size: usize,\n")
    f.write("    source: usize,\n")
    f.write("    destination: usize,\n")
    f.write("    graph: [GRAPH_SIZE][GRAPH_SIZE]i32,\n")
    f.write("};\n\n")
    f.write("pub var dijkstrasTestDataArray = [_]GraphData{\n")
    ...
    for g in graphs:
        ...
        f.write(f"        .source = {g['source']},\n")
        f.write(f"        .destination = {g['destination']},\n")
        f.write("        .graph = .{\n")  # Changed to use anonymous array literals
        
        for row in g["matrix"]:
            # Format each row properly with the [GRAPH_SIZE]i32 type and values
            row_str = ", ".join(str(x) for x in row)
            f.write(f"            .{{ {row_str} }},\n")  # Using anonymous array literals
