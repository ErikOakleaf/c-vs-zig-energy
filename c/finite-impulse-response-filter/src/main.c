...
void firFilterInt(const uint32_t in[], uint32_t out[], uint32_t inputLength, const uint32_t coef[], uint32_t coefLength, uint32_t scale) {
    ...
    const uint32_t *coefStart = coef;
    uint32_t coefLength2 = (coefLength + 1) >> 1;

    // set up input data pointers 
    const uint32_t *inEnd = in + inputLength - 1;
    const uint32_t *inPtr = in + coefLength2 - 1;
    ...
    for (int i = 0; i < inputLength; i++) {
        ...
        const uint32_t *dataPtr = inPtr;
        const uint32_t *coefPtr = coefStart;

        // do accumulation and write result with scale factor 

        uint32_t acc = (uint32_t)(*coefPtr++) * (*dataPtr--);
        for (int j = 1; j < accLength; j++)
            acc += (uint32_t)(*coefPtr++) * (*dataPtr--);
        *out++ = (int)(acc / scale);
        ...
        if (inPtr == inEnd) {
            ...
            coefStart++; //next coefficient each time 
        }
